        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active"> Ανοιχτά tickets</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
...
<?php // Free result set
   ...
   <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Όλα τα ανοιχτά tickets </h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
 ...
 */
  ...
  Category as Category, DisplayName as DisplayName, (SELECT Usertype from users where WhoOpenedIt=UserID) Usertype, TDescription as Description, Office as Office,
  (SELECT DisplayName from users where WhoOpenedIt=UserID) WhoOpenedIt from tickets left join users on FKUserID=UserID left join status on FKStatus=StatusID  
  where StatusText='Νέο' or StatusText='Αναμονή απάντησης' or StatusText='Έγινε επίσκεψη')
  UNION
  (select OTicketNumber as TicketNumber, OSubject as Subject, OProblem as Problem, OCreationDate as CreationDate, 
  StatusText as status, OCategory as Category, ODisplayName as DisplayName, (SELECT Usertype from users where OWhoOpenedIt=UserID) Usertype,
  ODescription as Description, OPriority as Priority,
  OOffice as Office, (SELECT DisplayName from users where OWhoOpenedIt=UserID) WhoOpenedIt from othertickets  left join users on OFKUserID=UserID left join status on OFKStatus=StatusID 
  where StatusText='Νέο' or StatusText='Αναμονή απάντησης' or StatusText='Έγινε επίσκεψη')) ola ";
 $conditions = array();
...
?>
 <div class="table-responsive" >
      ...
      </div> <div class="table-responsive" hidden><?php
				}
...
?>					<table id="ola"class="table table-bordered table-hover">
        ...
        <tbody>
			  <?php
			        $sql = "select * from((select TicketNumber as TicketNumber, Subject as Subject, Problem as Problem, CreationDate as CreationDate, StatusText as status,
  ...
  UNION 
  (select OTicketNumber as TicketNumber, OSubject as Subject, OProblem as Problem, OCreationDate as CreationDate, StatusText as status, 
  OCategory as Category, ODisplayName as DisplayName, 
  (SELECT Usertype from users where OWhoOpenedIt=UserID) Usertype, 
(SELECT OnomaDiefth from diefthinseismakedonias where DMID=ODescription) Description,
  OOffice as Office, (SELECT DisplayName from users where OWhoOpenedIt=UserID) WhoOpenedIt from othertickets left join users on OFKUserID=UserID left join status on 
  OFKStatus=StatusID)) ola
  where status='Νέο' or status='Αναμονή απάντησης' or status='Έγινε επίσκεψη'";
 if($result = mysqli_query($link, $sql)){
                        if(mysqli_num_rows($result) > 0){
                                while($row = mysqli_fetch_array($result)){
								  $_GET['TicketNumber'];
   ...
   // $assignstaff = '';
    ...
    foreach($result as $row){
       $assignstaff .= '<li value="'.$row['DisplayName'].'">'.$row['DisplayName'].'</li>';
