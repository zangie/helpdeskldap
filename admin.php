...
<br><br>
	   ...
	   <button onclick="killSession()">Διακοπή Εργασιών</button>
      <!-- /.sidebar-menu -->
    </div>
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active"> ΔΙΑΧΕΙΡΙΣΗ</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
...
<?php // Free result set
   ...
   <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Εβδομαδιαία tickets </h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
...
<table id="ola"class="table table-bordered table-hover">
 ...
 from othertickets order by statusid asc)) ola 
   where statusid in (0,1,2,3,4,5)
                or (statusid = 6 AND CreationDate >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)) order by statusid asc;";
 if($result = mysqli_query($link, $sql)){
                        if(mysqli_num_rows($result) > 0){
                                while($row = mysqli_fetch_array($result)){
									$_GET['TicketNumber'];
