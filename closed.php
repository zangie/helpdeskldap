        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Κλειστά tickets</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
...
<?php // Free result set
   ...
   <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Κλειστά tickets</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
        ...
        <tbody>
			  ...
			  <?php
			        $sql = "select * from((select TicketNumber as TicketNumber, 
               Subject as Subject,
               Problem as Problem, 
               ...
               (SELECT Usertype from users where WhoOpenedIt=UserID) Usertype, 
               TDescription as Description, 
               Office as Office,
  (SELECT DisplayName from users where WhoOpenedIt=UserID) WhoOpenedIt 
               from tickets left join users on FKUserID=UserID left join status on FKStatus=StatusID  
)
  UNION
  (select 
   OTicketNumber as TicketNumber, 
   OSubject as Subject, 
   OProblem as Problem, 
   OCreationDate as CreationDate, 
   StatusText as status, 
   OPriority as Priority,
   OCategory as Category, 
   ODisplayName as DisplayName, 
 ...
 (SELECT OnomaDiefth from diefthinseismakedonias where DMID=ODescription) Description, 
   OOffice as Office, (SELECT DisplayName from users where OWhoOpenedIt=UserID) WhoOpenedIt
   from othertickets left join users on OFKUserID=UserID left join status on OFKStatus=StatusID)) ola  where status='Κλειστό'; ";
 if($result = mysqli_query($link, $sql)){
                        if(mysqli_num_rows($result) > 0){
                                while($row = mysqli_fetch_array($result)){
									$_GET['TicketNumber'];
