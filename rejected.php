        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active"> Έχουν απορριφθεί</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
...
$sql='select * from ((select TicketNumber , FKStatus as FKStatus from tickets) union (select OTicketNumber, OFKStatus as FKStatus from othertickets)) 
ola where FKStatus="2" or FKStatus="0" or  FKStatus="1";
';
...
<?php // Free result set
   ...
   <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Έχουν απορριφθεί
 </h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
        ...
        <tbody>
			  ...
			  <?php
			        $sql = "select * from
	((select TicketNumber as TicketNumber, Subject as Subject, Problem as Problem, CreationDate as CreationDate,
	StatusText as status, Priority as Priority,
  ...
  OCategory as Category, ODisplayName as DisplayName, (SELECT Usertype from users where OWhoOpenedIt=UserID) Usertype,
(SELECT OnomaDiefth from diefthinseismakedonias where DMID=ODescription) Description, 
  OOffice as Office, (SELECT DisplayName from users where OWhoOpenedIt=UserID) WhoOpenedIt from othertickets  left join users on OFKUserID=UserID
  left join status on OFKStatus=StatusID )) ola  where status='Απορρίπτεται' ";
 if($result = mysqli_query($link, $sql)){
                        if(mysqli_num_rows($result) > 0){
                                while($row = mysqli_fetch_array($result)){
									$_GET['TicketNumber'];
	if (($row['TicketNumber'][0])==="a"){
	 ...
	 id="ticketnumber_' . $row["TicketID"] . '" data-type="text" data-pk = "' . $row["TicketID"] . 
						...
						echo "<td>". $row['Subject'] ."-<br><strong>". $row['Problem']. "</strong></td>";
								...
								if ($row['status']== 'Waiting Reply' && $row['ReplyID']==null){
