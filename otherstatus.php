        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active"> Έχουν απαντηθεί/Σε εξέλιξη/Χρειάζεται επίλυση από τρίτο μέρος
</li>
            </ol>
          </div>
        </div>
...
<?php // Free result set
   ...
   <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Έχουν απαντηθεί/Σε εξέλιξη/Χρειάζεται επίλυση από τρίτο μέρος </h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
 ...
 <div class="table-responsive" >
  ...
  StatusText as status, OPriority as Priority, OCategory as Category, ODisplayName as DisplayName, (SELECT Usertype from users where OWhoOpenedIt=UserID) Usertype,
 (SELECT OnomaDiefth from diefthinseismakedonias where DMID=ODescription) Description,  OOffice as Office, (SELECT DisplayName from users where OWhoOpenedIt=UserID) WhoOpenedIt from othertickets  left join users on OFKUserID=UserID
  left join status on OFKStatus=StatusID )) ola  where status='Έχει απαντηθεί' or status='Σε εξέλιξη' or status='Χρειάζεται επίλυση από τρίτο μέρος'";
 if($result = mysqli_query($link, $sql)){
                        if(mysqli_num_rows($result) > 0){
                                while($row = mysqli_fetch_array($result)){
									$_GET['TicketNumber'];
